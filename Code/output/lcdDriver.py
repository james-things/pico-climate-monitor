...
# the output is 480x320
# the pins for the output are defined by the hardware and fixed
# --------------------------------------------------------------------------
...
class LCD3inch5:
    ...
    def __init__(self):
        ...
        # the high speeds work fine with the output but the touch needs a much slow spi
        # touch=4MHz, output=64MHz
    ...
    def _init_display(self):
        ...
        sleep_ms(120)
        self._writeCommand(0x29)  # output on
        self._writeCommand(0xB6, b"\x00\x62")  # output function
        self._writeOne(0x36, 0x28)  # memory access control - rowcol exchange, BGR
    ...
    def show_area(self, xMin, yMin, xMax, yMax):
        ''' copy the framebuffer into a rectangular piece of the output '''
        xMin = int(xMin)
        xMax = int(xMax)
        yMin = int(yMin)
        yMax = int(yMax)
    ...
    def draw_line(self, x0, y0, x1, y1, color, bg_color):
        ...
        self.set_frame_size(x_max - x_min + 1, y_max - y_min + 1)
        self._MyFrame.fill(bg_color)
    ...
    def draw_string_box(self, drawer: FontDrawer, text, toffset, x, y, width, height, txtColor, bkgdColor, shrink):
        ...
        LCD = self._MyFrame
        LCD.fill(bkgdColor)
        drawer.DrawString(text, toffset, 0, txtColor, shrink)
