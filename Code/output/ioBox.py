...
class IoBox:
    ...
    def __init__(self, oled, font, width=0, height=0, shrink=False, just=None, cached=False):
    ...
    def Draw(self, xpos, ypos):
        ...
        if not self.usecache:
            self.oled.draw_string_box(self.drawer, self.text, xoffset, xpos, ypos, self.width, self.height,
                                      self.foreColor, self.backColor, self.shrink)
        else:
            self.oled.draw_string_cached(self.drawer, self.text, xoffset, xpos, ypos, self.width, self.height,
                                         self.backColor)
